...
#[command(
    ...
    after_help = "ENVIRONMENT VARIABLES:\n  \
        ...
        QUICKSTART:\n  \
        veen keygen --out ./client\n  \
        veen hub start --listen 127.0.0.1:8080 --data-dir ./hub\n  \
...
/// Fast-path handler for top-level version flags without building the clap command tree.
...
/// Entry point for CLI execution, including a fast-path for top-level version output.
...
async fn send_message_remote(client: HubHttpClient, args: SendArgs) -> Result<SendOutcome> {
    ...
    let cbor_request = SubmitCborRequest {
...
async fn handle_stream_remote(client: HubHttpClient, args: StreamArgs) -> Result<()> {
    let response =
        fetch_stream_cbor(&client, &args.stream, args.from, args.to, args.with_proof).await?;
...
async fn fetch_stream_cbor(
    ...
    if response.ver != DATA_PLANE_VERSION {
        bail!(
            "hub returned unsupported stream response version {}",
            response.ver
        );
    }
    Ok(response)
...
async fn execute_federate_mirror_run(
    ...
    while next_seq <= upto_seq {
        let response =
            fetch_stream_cbor(&context.source_client, &source_label, next_seq, None, true)
                .await
                .with_context(|| {
                    format!(
...
async fn fetch_stream_messages(
    ...
    match reference {
        ...
        HubReference::Remote(client) => {
            let result = fetch_stream_cbor(client, stream, 0, None, false).await;
            match result {
